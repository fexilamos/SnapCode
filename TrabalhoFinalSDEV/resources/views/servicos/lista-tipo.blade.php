...
@section('content')
    ...
    <main class="px-4 md:px-8 font-mono">
        <div class="max-w-7xl mx-auto">
            <!-- Filtro de pesquisa -->
            <form method="GET" action="{{ route('servicos.tipo', ['tipo' => $tipo]) }}" class="w-full flex flex-col gap-6 mb-12 bg-slate-800 rounded-xl p-8 shadow-lg border border-slate-600 font-mono">
            ...
            <div class="bg-slate-700 rounded-xl p-8 border border-slate-600 mt-10 shadow-xl font-mono">
                        ...
                        <tbody>
                            @forelse($servicos as $servico)
                                <tr class="border-b border-slate-600 hover:bg-slate-600 transition font-mono">
                                    <td class="px-4 py-2 font-mono">{{ $servico->nome_servico }}</td>
                                    <td class="px-4 py-2 font-mono">{{ $servico->cliente->nome ?? '-' }}</td>
                                    <td class="px-4 py-2 font-mono">{{ \Carbon\Carbon::parse($servico->data_inicio)->format('d/m/Y') }}</td>
                                    <td class="px-4 py-2 font-mono">{{ $servico->localizacao->nome_local ?? '-' }}</td>
                ...
                <div class="mt-10 pt-6 border-t border-slate-600 flex justify-center">
                    {{ $servicos->appends(request()->except('page'))->links('pagination::tailwind') }}
                </div>
            </div>

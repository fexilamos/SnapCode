...
@section('content')
    ...
    <div class="min-h-screen flex flex-col items-center justify-center py-12">
        ...
        <div class="w-full max-w-5xl">
            ...
            {{-- FORM DE EDIÇÃO --}}
            <form method="POST" action="{{ route('servicos.checkout.update', $servico->cod_servico) }}">
                @csrf
                @method('PUT')
                            ...
                            <div
                                ...
                                <div class="flex-1 w-full">
                                    <select name="funcionarios[]"
                                        class="form-select w-full rounded-xl border-slate-600 px-4 py-3 bg-slate-800 text-white shadow funcionario-select"
                                        onchange="actualizarFuncoes(this)" required>
                                        <option value="">Escolha o funcionário</option>
                                        @foreach ($funcionarios as $f)
                                            <option value="{{ $f->cod_funcionario }}"
                                                {{ $cod_funcionario == $f->cod_funcionario ? 'selected' : '' }}>
                                                {{ $f->nome }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <select name="funcoes[]"
                                        class="form-select mt-3 w-full rounded-xl border-slate-600 px-4 py-3 bg-slate-800 text-white shadow funcao-select"
                                        required>
                        ...
                        @foreach ($kitsSelecionados as $selectedKit)
                            <div
                                ...
                                <div class="flex flex-col md:flex-row gap-4 items-center w-full">
                                    <select name="kits[]"
                                        class="form-select w-full rounded-xl border-slate-600 px-4 py-3 bg-slate-800 text-white shadow kit-select"
                                        required>
        ...
        function actualizarOpcoesKits() {
            ...
            document.querySelectorAll('.kit-select').forEach(function(select) {
                Array.from(select.options).forEach(function(option) {
                    ...
                    option.disabled = todosSelecionados.includes(option.value) && select.value !== option
                        .value;
                });
            });

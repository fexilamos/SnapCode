...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-base text-slate-100">
    ...
    <div><span class="font-semibold text-slate-400">Hora chegada casa bebê:</span> {{ $detalhes->hora_chegada_casa_bebe ?: '—' }}</div>
    <div><span class="font-semibold text-slate-400">Hora saída casa bebê:</span> {{ $detalhes->hora_saida_casa_bebe ?: '—' }}</div>
    <div><span class="font-semibold text-slate-400">Nome do bebê:</span> {{ $detalhes->nome_bebe ?: '—' }}</div>
    <div><span class="font-semibold text-slate-400">Morada do bebê:</span> {{ $detalhes->morada_bebe ?: '—' }}</div>
    <div><span class="font-semibold text-slate-400">Agregado do bebê:</span> {{ $detalhes->agregado_bebe ?: '—' }}</div>
    <div><span class="font-semibold text-slate-400">Info extra bebê:</span> {{ $detalhes->info_extra_bebe ?: '—' }}</div>

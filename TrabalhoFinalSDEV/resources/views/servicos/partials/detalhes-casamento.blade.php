...
<div class="space-y-8">
    ...
    <div class="rounded-xl bg-slate-900/80 shadow p-6 border border-slate-800">
        <h4 class="text-lg font-semibold mb-4 border-l-4 pl-4 text-white tracking-wide">Noivo</h4>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-3 text-slate-100">
            <div><span class="font-semibold text-slate-400">Hora Chegada:</span> {{ $detalhes->hora_chegada_casa_noivo ?: '—' }}</div>
            <div><span class="font-semibold text-slate-400">Hora Saída:</span> {{ $detalhes->hora_saida_casa_noivo ?: '—' }}</div>
            <div><span class="font-semibold text-slate-400">Nome:</span> {{ $detalhes->nome_noivo ?: '—' }}</div>
            <div><span class="font-semibold text-slate-400">Morada:</span> {{ $detalhes->morada_noivo ?: '—' }}</div>
            <div><span class="font-semibold text-slate-400">Agregado Familiar:</span> {{ $detalhes->agregado_noivo ?: '—' }}</div>
            <div class="md:col-span-2"><span class="font-semibold text-slate-400">Informações Extra:</span> {{ $detalhes->info_extra_noivo ?: '—' }}</div>
        </div>
    </div>

    {{-- Noiva --}}
    <div class="rounded-xl bg-slate-900/80 shadow p-6 border border-slate-800">
        <h4 class="text-lg font-semibold mb-4 border-l-4 pl-4 text-white tracking-wide">Noiva</h4>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-3 text-slate-100">
            <div><span class="font-semibold text-slate-400">Hora Chegada:</span> {{ $detalhes->hora_chegada_casa_noiva ?: '—' }}</div>
            <div><span class="font-semibold text-slate-400">Nome:</span> {{ $detalhes->nome_noiva ?: '—' }}</div>
            <div><span class="font-semibold text-slate-400">Morada:</span> {{ $detalhes->morada_noiva ?: '—' }}</div>
            <div><span class="font-semibold text-slate-400">Agregado Familiar:</span> {{ $detalhes->agregado_noiva ?: '—' }}</div>
            <div class="md:col-span-2"><span class="font-semibold text-slate-400">Informações Extra:</span> {{ $detalhes->info_extra_noiva ?: '—' }}</div>
        </div>
    </div>
    ...
    <div class="rounded-xl bg-slate-900/80 shadow p-6 border border-slate-800">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-3 text-slate-100">
            ...
            <div><span class="font-semibold text-slate-400">Instruções Saída:</span> {{ $detalhes->instrucoes_saida_igreja ?: '—' }}</div>

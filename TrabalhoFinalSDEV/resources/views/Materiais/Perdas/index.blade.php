...
<div class="container mx-auto px-4">
    <h2 class="text-2xl text-white font-bold mb-6">Consultar Perdas</h2>

    <!-- Botão Registar Nova Perda -->
    <div class="flex justify-end mb-8 mt-6"> 
        <a href="{{ route('perdas.create') }}" class="bg-green-600 text-white px-6 py-3 rounded hover:bg-green-700 font-semibold transition-colors shadow">
            Registar Nova Perda
        </a>
    </div>
    <div class="bg-gray-100 rounded-xl shadow p-6 mb-8">
        ...
        @endif
        <form method="GET" action="{{ route('perdas.index') }}" class="mb-4">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full bg-white rounded-xl shadow mt-4">
                <thead class="bg-gray-200">
                    <tr>
                        <th class="py-2 px-4 text-black">ID Perda</th>
                ...
                <tbody>
                    @forelse($perdas as $perda)
                        ...
                        <tr class="border-b hover:bg-gray-50">
                            <td class="py-2 px-4 text-black">{{ $perda->cod_perda }}</td>
                            <td class="py-2 px-4 text-black">{{ $perda->material->num_serie ?? '-' }}</td>
                            <td class="py-2 px-4 text-black">{{ $perda->material->categoria->categoria ?? '-' }}</td>
                            <td class="py-2 px-4 text-black">{{ $perda->material->marca->marca ?? '-' }}</td>
                            <td class="py-2 px-4 text-black">{{ $perda->material->modelo->modelo ?? '-' }}</td>
                            <td class="py-2 px-4 text-black">{{ $perda->material->estado->estado_nome ?? '-' }}</td>
                            <td class="py-2 px-4 text-black">{{ $perda->observacoes }}</td>
                            <td class="py-2 px-4 text-black">{{ $perda->data_registo ? \Carbon\Carbon::parse($perda->data_registo)->format('d/m/Y') : '-' }}</td>
                            <td class="py-2 px-4 flex flex-col gap-1 md:flex-row md:gap-2 justify-center">
                                <a href="{{ route('perdas.edit', $perda->cod_perda) }}" class="bg-yellow-500 text-white px-3 py-1 rounded hover:bg-yellow-600 text-xs">Editar</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="9" class="py-4 text-center text-gray-500">Nenhuma perda registada.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>

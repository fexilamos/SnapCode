...
<main class="px-4 md:px-8">
    <div class="max-w-7xl mx-auto">
        <div class="flex justify-end mb-8 mt-6">
            <a href="{{ route('perdas.create') }}" class="bg-slate-700 text-white px-6 py-3 rounded hover:bg-slate-500 font-semibold transition-colors shadow">
                Registar Nova Perda
            </a>
        </div>
        <div class="bg-slate-700 rounded-xl p-8 border border-slate-600 shadow-lg">
            ...
            <form method="GET" action="{{ route('perdas.index') }}" class="w-full flex flex-col gap-6 mb-12 bg-slate-800 rounded-xl p-8 shadow-lg border border-slate-600">
                ...
                <div class="flex flex-col sm:flex-row gap-4 items-center justify-center w-full pt-4">
                    ...
                    <a href="{{ route('perdas.index') }}" class="px-8 py-3 bg-slate-600 hover:bg-slate-700 text-white rounded-lg font-semibold shadow transition-all duration-200 text-center">Limpar Filtros</a>
                </div>
            </form>
                    ...
                    <tbody class="divide-y divide-slate-600">
                        @forelse($perdas as $perda)
                            <tr class="hover:bg-slate-800 transition-colors {{ $loop->even ? 'bg-slate-700' : 'bg-slate-600' }}">
                                ...
                                <td class="px-4 py-3 flex flex-col gap-1 md:flex-row md:gap-2 justify-center">
                                    <a href="{{ route('perdas.edit', $perda->cod_perda) }}" class="bg-slate-700 text-white px-3 py-1 rounded hover:bg-slate-600 text-xs">Editar</a>
                                </td>
                            </tr>
                        @empty
                            <tr>
